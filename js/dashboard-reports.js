...
window.processStats = (snapshot) => {
    ...
    let pisoWifiGross = 0, pisoWifiPayouts = 0, printingGross = 0, coffeeGross = 0;
    ...
    if (document.getElementById('pisonetSummaryContainer')) document.getElementById('pisonetSummaryContainer').innerHTML = Object.entries(pisonetTotals).map(([br, amt]) => `<div class="flex justify-between items-center text-xs"><span>PISONET (${br})${getDaysAgo(pisonetLastDates[br])}</span><span class="font-bold text-white">₱${amt.toFixed(2)}</span></div>`).join('');
...
window.renderPerformanceChart = (snapshot, targetYear, branchFilter) => {
    ...
    const dataset = (label, data, color) => ({ label, data, borderColor: color, backgroundColor: color, tension: 0.4, pointRadius: 2, borderWidth: 2 });
    const data = { labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], datasets: [dataset('Pisonet', pData, '#3b82f6'), dataset('PisoWiFi', wData, '#f97316'), dataset('Coffee', cData, '#fbbf24'), dataset('Printing', prData, '#00e676')] };
    if (performanceChartInstance) { performanceChartInstance.data = data; performanceChartInstance.update(); }
    else { performanceChartInstance = new Chart(ctx, { type: 'line', data, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: 'rgba(255,255,255,0.7)', font: { size: 9 } } } }, scales: { x: { ticks: { color: 'rgba(255,255,255,0.5)', font: { size: 8 } }, grid: { display: false } }, y: { beginAtZero: true, ticks: { color: 'rgba(255,255,255,0.5)', font: { size: 8 }, callback: (v) => '₱' + v }, grid: { color: 'rgba(255, 255, 255, 0.05)' } } } } }); }
};
