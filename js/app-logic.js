...
// Global State - Attached to window for all modules to see
window.appSettings = {
...
window.saveDeduction = async () => {
    ...
    if (isNaN(inputVal) || inputVal <= 0 || inputVal > maxVal) {
        return alert("Please enter a valid amount up to ₱" + maxVal.toFixed(2));
    }

    try {
        const newBalance = maxVal - inputVal;
        await setDoc(doc(db, "jgs_logs", currentDeductionTarget), { pendingBalance: newBalance }, { merge: true });
